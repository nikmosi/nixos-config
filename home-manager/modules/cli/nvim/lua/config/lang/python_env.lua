...
local function get_python_dir(root_dir)
  -- Сначала уважаем UV_PROJECT_ENVIRONMENT (его выставляют devenv/uv)
  local uv_env = vim.env.UV_PROJECT_ENVIRONMENT
  if uv_env and vim.uv.fs_stat(uv_env) then
    return uv_env
  end
...
M.env = function(root_dir)
  ...
  if not _virtual_env then
    vim.notify("can't find venv, falling back to system python", vim.log.levels.WARN)
